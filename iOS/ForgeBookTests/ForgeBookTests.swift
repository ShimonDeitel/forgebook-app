import XCTest
@testable import ForgeBook

@MainActor
final class ForgeBookTests: XCTestCase {
    ...
    func testAddIncreasesCount() {
        ...
        store.add(Session(title: "Test Entry"))
        XCTAssertEqual(store.items.count, before + 1)
    }

    func testAddInsertsAtFront() {
        let store = Store()
        store.add(Session(title: "Newest"))
        XCTAssertEqual(store.items.first?.title, "Newest")
    }

    func testDeleteRemovesItem() {
        let store = Store()
        store.add(Session(title: "ToDelete"))
    ...
    func testCannotAddAtFreeLimitWhenNotPro() {
        ...
        while store.items.count < Store.freeLimit {
            store.add(Session(title: "Filler"))
        }
    ...
    func testUpdateModifiesExistingItem() {
        let store = Store()
        store.add(Session(title: "Original"))
